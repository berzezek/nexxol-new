...
describe("/api/example/:id", () => {
	it("GET returns 404 when id does not exist", async () => {
		const result = await $fetch("/api/example/1", {}).catch(
			(error) => error.data
		);
		expect(result).toStrictEqual({
	...
	it("PUT creates an entity, then GET allows to fetch it", async () => {
		...
		const exampleAfterUpsert = await $fetch(
			`/api/example/${faker.datatype.uuid()}`,
			{
				method: "PUT",
		...
		expect(Object.keys(exampleAfterUpsert).sort()).toEqual(
			["id", "description", "details"].sort()
		);
		...
		const exampleFromGet = await $fetch(
			`/api/example/${exampleAfterUpsert.id}`
		);
	...
	it("PUT creates and entity if none exists, then updates it on a second call", async () => {
		...
		const updatePayload: Prisma.ExampleUpdateInput = {
			description: faker.lorem.paragraphs(),
			details: faker.lorem.paragraphs(),
		};
		const exampleAfterUpdate = await $fetch(
			`/api/example/${faker.datatype.uuid()}`,
			{
				method: "PUT",
				body: updatePayload,
